...
Проанализируй, заполнен ли документ (или его поля) от руки (курсивом или ручкой).
1. Внимательно посмотри на поля ФИО, Даты и АДРЕСА РЕГИСТРАЦИИ (штамп прописки).
2. Если адрес в штампе написан ручкой (курсив) -> **isHandwritten: true**.
3. Если ФИО или даты вписаны ручкой -> **isHandwritten: true**.
4. Подписи должностных лиц НЕ считаются рукописным текстом.
5. Если весь документ печатный -> **isHandwritten: false**.
...
2. **ПРОПИСКА (ШТАМП РЕГИСТРАЦИИ) - ЧИТАТЬ ВЕСЬ БЛОК!**:
   ...
   Если адрес вписан от руки - не забудь поставить **isHandwritten: true**.
